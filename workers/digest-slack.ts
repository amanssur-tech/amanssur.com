...
export async function main(
  env: { SUBMISSIONS: KVNamespace; [key: string]: unknown },
  params?: URLSearchParams | Record<string, string | undefined>,
) {
  ...
  const since =
    ...
    new Date(
      Date.now() -
        (args.hours && args.hours > 0 ? args.hours : 24) * 60 * 60 * 1000,
    );
  ...
  await sendSlackMessage(msg, env as Record<string, string>);
